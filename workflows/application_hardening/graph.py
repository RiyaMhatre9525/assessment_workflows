"""
StateGraph wiring for the application_hardening workflow.
"""
...
class ApplicationHardeningState(TypedDict, total=False):
    # Inputs
    assessment_id: str
...
class ApplicationHardeningWorkflow(BaseWorkflow):
    """Assesses application security hardening maturity across VCS + cloud."""

    def __init__(self):
        super().__init__(
            name="Application Hardening Assessment",
            description=(
                "Assesses application security hardening maturity across connected "
                "version control (GitHub, Azure DevOps) and cloud (Azure) platforms "
                "using 5 progressive, fail-fast levels aligned to OWASP ASVS/MASVS."
            ),
        )
        self.domain_name = "application_hardening"

    def build_graph(self):
        graph = StateGraph(ApplicationHardeningState)
    ...
    def initialize_state(self, input_data: dict) -> dict:
        ...
        return {
            ...
            "cloud_resource_scope": input_data.get("cloud_resource_scope", input_data["repository"]),
            "level_results": {},
